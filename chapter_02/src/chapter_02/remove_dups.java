...
public class remove_dups {
	...
	/**
	 * 
	 * This solution loops the current list and if the item is not in the new
	 * linkedlist used as a buffer it is then added. After looping through all items
	 * the new linked list is returned where the current list can be updated to the
	 * new one.
	 * 
	 * @param list
	 * @return
	...
	/**
	 * 
	 * Loops through list and then nested loop loops through remaining item by
	 * starting at the next index from current word in list. If the two values at i
	 * and j (index) are the same value at j is removed. The counter j is then
	 * reduced by one for the removed item and so that the next item is not misssed.
	 * 
	 * @param list
